...
/**
 * [upperBound] is the highest annual income, in rupees, that still falls in the slab — the top
 * slab is open-ended and uses [Long.MAX_VALUE]. The user types an amount and [forAmount] maps it
 * to the slab, so the request still carries the [apiValue] the API has always expected.
 */
enum class IncomeSlab(val apiValue: String, val displayName: String, val upperBound: Long) {
    UPTO_1_LAKH("upto_1lakh", "Up to ₹1 Lakh", 1_00_000L),
    ABOVE_1_UPTO_5_LAKH("above_1lakh_upto_5lakh", "₹1 Lakh - ₹5 Lakh", 5_00_000L),
    ABOVE_5_UPTO_10_LAKH("above_5lakh_upto_10lakh", "₹5 Lakh - ₹10 Lakh", 10_00_000L),
    ABOVE_10_UPTO_25_LAKH("above_10lakh_upto_25lakh", "₹10 Lakh - ₹25 Lakh", 25_00_000L),
    ABOVE_25_LAKH_UPTO_1_CR("above_25lakh_upto_1cr", "₹25 Lakh - ₹1 Crore", 1_00_00_000L),
    ABOVE_1_CR("above_1cr", "Above ₹1 Crore", Long.MAX_VALUE);

    companion object {
        /** Boundaries are inclusive at the top, matching the "up to" wording of the labels. */
        fun forAmount(annualIncome: Long): IncomeSlab =
            entries.first { annualIncome <= it.upperBound }
    }
}
