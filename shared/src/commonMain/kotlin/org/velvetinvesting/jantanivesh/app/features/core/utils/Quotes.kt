package org.velvetinvesting.jantanivesh.app.features.core.utils

val loadingQuotes = listOf(

    "Financial freedom isn’t a dream.\nIt’s a plan you execute daily.",

    "Every investment brings you closer.\nFIRE is built step by step.",

    "Your future self is funded by today’s discipline.\nStay consistent.",

    "Wealth grows quietly.\nConsistency is louder than luck.",

    "Retire early, live fully.\nLet your money buy your freedom.",

    "Goals are just numbers.\nExecution turns them into reality.",

    "Don’t chase quick gains.\nBuild lasting wealth.",

    "Freedom is having choices.\nInvest to create them.",

    "Small steps today.\nMassive financial freedom tomorrow.",

    "Velvet helps you plan.\nYou make it happen."
)